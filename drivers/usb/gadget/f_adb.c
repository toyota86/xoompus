				...
				size_t count, loff_t *pos)
{
	...
	if (count > BULK_BUFFER_SIZE) {
		printk(KERN_ERR "%s called with count %d > BULK_BUFFER_SIZE",
				 __func__, count);
		return -EINVAL;
	}
...
requeue_req:
	...
	if (r != count)
		printk(KERN_ERR "%s returning %d for count %d\n", __func__, r, count);
	return r;
}
...
static int adb_enable_open(struct inode *ip, struct file *fp)
{
	...
	mutex_lock(&_adb_dev->adb_enable_mutex);
...
#endif
	mutex_unlock(&_adb_dev->adb_enable_mutex);
	return 0;
}
#ifdef CONFIG_USB_MOT_ANDROID
...
static int adb_bind_config(struct usb_configuration *c)
{
	...
	int status;
#endif
	printk(KERN_INFO "Gadget Usb: adb_bind_config\n");
	...
	mutex_init(&dev->adb_enable_mutex);
