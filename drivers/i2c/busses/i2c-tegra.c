	...
	struct i2c_msg msgs[], int num)
{
	...
	if (i2c_dev->is_suspended) {
		dev_dbg(i2c_dev->dev, "%s: xfer while suspended.\n", __func__);
		return -EBUSY;
	}
	...
	rm_tx = kzalloc(num*sizeof(NvRmI2cTransactionInfo), GFP_ATOMIC);
	if (!rm_tx) {
	...
	if (len) {
		data = kzalloc(len*sizeof(NvU8), GFP_ATOMIC);
		if (!data) {
