...
struct sdhci_ops {
	...
	u32		(*m_readl)(struct sdhci_host *host, int reg);
	u16		(*m_readw)(struct sdhci_host *host, int reg);
	u8		(*m_readb)(struct sdhci_host *host, int reg);
	void		(*m_writel)(struct sdhci_host *host, u32 val, int reg);
	void		(*m_writew)(struct sdhci_host *host, u16 val, int reg);
	void		(*m_writeb)(struct sdhci_host *host, u8 val, int reg);
#endif
...
{
	if (unlikely(host->ops->m_writel))
		host->ops->m_writel(host, val, reg);
	else
...
{
	if (unlikely(host->ops->m_writew))
		host->ops->m_writew(host, val, reg);
	else
...
{
	if (unlikely(host->ops->m_writeb))
		host->ops->m_writeb(host, val, reg);
	else
...
{
	if (unlikely(host->ops->m_readl))
		return host->ops->m_readl(host, reg);
	else
...
{
	if (unlikely(host->ops->m_readw))
		return host->ops->m_readw(host, reg);
	else
...
{
	if (unlikely(host->ops->m_readb))
		return host->ops->m_readb(host, reg);
	else
