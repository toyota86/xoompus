...
static int gart_probe(struct platform_device *pdev)
{
	...
	int e;
	volatile unsigned int* ptr = (volatile unsigned int *)0xFE30f180;

	pr_err("\n**** MC_FPRI_CTRL_DC_0=0x%x ******", *ptr++);
	pr_err("\n**** MC_FPRI_CTRL_DCB_0=0x%x ******", *ptr);
	pr_err("chaning the MC_FPRI_CTRL_DC_0 to 0x3FF");
	pr_err("chaning the MC_FPRI_CTRL_DCB_0 to 0x3FF");
	ptr = (volatile unsigned int *)0xFE30f180;
	*ptr++ = 0x3FF;
	*ptr= 0x3FF;
	ptr = (volatile unsigned int *)0xFE30f180;
	pr_err("\n**** MC_FPRI_CTRL_DC_0=0x%x ******", *ptr++);
	...
	pr_err("\n**** MC_FPRI_CTRL_* dump ******\n");
	ptr = (volatile unsigned int *)0xFE30f17c;
	while (ptr <= (volatile unsigned int *)0xFE30f1b4) {
		pr_err("    %p: %x\n", ptr, *ptr);
		ptr++;
	}
